...
function lines() {
  ...
  var g = svg.selectAll('.symbol').attr('transform', function(d, i) {
...
function horizons() {
  ...
  var g = svg.selectAll('.symbol').attr('clip-path', 'url(#clip)');
...
function areas() {
  ...
  g.select('.line').attr('d', function(d) {
...
function stackedArea() {
  ...
  y.domain([
    0,
    d3.max(
      symbols[0].values.map(function(d) {
...
function overlappingArea() {
  ...
  y.domain([
    0,
    d3.max(
      symbols.map(function(d) {
        return d.maxPrice;
      }),
    ),
  ]).range([h, 0]);

  area.y0(h).y1(function(d) {
    return y(d.price);
  });
    ...
    return function(t) {
      ...
      text.attr(
        ...
        'translate(' +
          ...
          yy +
          ')rotate(' +
          (((f.startAngle + f.endAngle) / 2 + (3 * Math.PI) / 2) * 180) /
            Math.PI +
          ')',
      );
    };
      ...
      return function(t) {
        ...
        text.attr(
          ...
          'translate(' +
            ...
            h / 2 +
            ')rotate(' +
            (((a.startAngle + a.endAngle) / 2 + (3 * Math.PI) / 2) * 180) /
              Math.PI +
            ')',
        );
      };
